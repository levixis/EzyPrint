...
interface StudentOrderCardProps {
  ...
  /** Cancel is in flight — covers the payment-status check as well as the
   *  cancellation itself, since both are network round trips. */
...
const StudentOrderCard: React.FC<StudentOrderCardProps> = ({ order, onPayNow, onCancelOrder, isProcessingPayment, isCancelling }) => {
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-start mb-4 pb-4 border-b border-brand-muted/30">
        ...
        <div className="flex items-center gap-2 mt-2 sm:mt-0 flex-wrap">
          {order.isPremiumOrder && (
            <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full bg-gradient-to-r from-amber-400 to-yellow-500 text-amber-900 inline-flex items-center gap-1 shadow-sm">
          ...
          <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${statusStyle.border} ${statusStyle.text} ${statusStyle.bg} capitalize inline-flex items-center`}>
            {statusStyle.icon}
            {formattedStatus}
          </span>
        </div>
